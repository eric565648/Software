...
grip_sub = message_filters.Subscriber("/grip_controller/state", dynamixel_msgs.msg.JointState)
joint_pub = rospy.Publisher("/joint_states", sensor_msgs.msg.JointState,  queue_size=1) 

def joint_callback(j1, j2, g):
    #print j1, j2
    js = sensor_msgs.msg.JointState( header = j1.header, 
                                     name = ['joint1', 'joint2', 'grip'],
                                     position = [-j1.current_pos, -j2.current_pos, -g.current_pos],
                                     velocity = [-j1.velocity, -j2.velocity, -g.velocity],
                                     effort = [j1.load, j2.load, g.load] )
    joint_pub.publish(js)
    
ts = message_filters.ApproximateTimeSynchronizer([joint1_sub, joint2_sub, grip_sub], 20, 1, 1)
